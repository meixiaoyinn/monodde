...
class KittiDataset:
    '''kitti Dataset '''
    def __init__(self,root,cfg,augment=True):
        self.cfg = cfg  # setup
        self.root = root
        ...
        self.augmentation = get_composed_augmentations() if (self.is_train and augment) else None  # Only RandomHorizontallyFlip.
    ...
    def __getitem__(self, idx):
        ...
        if self.enable_edge_fusion:
            ...
            edge_indices = self.get_edge_utils((img_w, img_h), pad_size)  # edge_indices: The coordinates of edges in anticlockwise order.
        ...
        if self.orientation_method == 'head-axis':
            ...
            orientations = np.zeros([self.max_objs, self.multibin_size * 2],dtype=np.float32)  # multi-bin loss: 2 cls + 2 offset
