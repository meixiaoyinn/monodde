...
from model_utils.utils import get_iou_3d
...
def Real_MultiBin_loss(vector_ori, gt_ori, num_bin=4):
    ...
    for i in range(num_bin):
        # bin cls loss
        cls_ce_loss = ops.cross_entropy(vector_ori[:, (i * 2): (i * 2 + 2)], ops.expand_dims(gt_ori[:, i], -1),
...
class Mono_loss(nn.Cell):
    ...
    def construct(self, targets_heatmap, predictions, pred_targets, preds, reg_nums, weights, iteration):
        ...
        if self.heatmap_type == 'centernet':
            ...
            hm_loss = self.loss_weights['hm_loss'] * hm_loss  # Heatmap loss.
        ...
        if num_reg_2D > 0:
            ...
            iou_2D = iou_2D.mean()
            depth_MAE = (preds['depth_3D'] - pred_targets['depth_3D']).abs() / pred_targets['depth_3D']  # MAE for direct depth regression.

        objs_IOU =get_iou_3d(preds['corners_3D'], pred_targets['corners_3D'])  # objects_IOU shape: (val_objs,)
        ...
        if self.compute_IOU3D_predict_loss:
            ...
            IOU3D_predict_loss = ops.binary_cross_entropy_with_logits(preds['IOU3D_predict'].squeeze(1), IOU_label,
                                                                      ops.Tensor(np.array([1.0, 1.0, 1.0]), ms.float32),
                                                                      reduction='none')
        ...
        if num_reg_3D > 0:
            ...
            if self.compute_corner_loss:
                ...
                corner_3D_loss = self.loss_weight_ramper(iteration) * self.loss_weights['corner_loss'] * corner_3D_loss.mean()
            ...
            if self.compute_keypoint_corner:
                if self.corner_offset_uncern:
                    ...
                    keypoint_loss_mask = ops.cast(ops.expand_dims(pred_targets['keypoints_mask'], 2).expand_as(keypoint_loss), ms.bool_)  # keypoint_loss_mask shape: (val_objs, 10, 2)
                    ...
                    if self.cfg.SOLVER.DYNAMIC_WEIGHT:
                        keypoint_objs_weight = ops.expand_dims(ops.expand_dims(objs_weight, 1), 2).expand_as(keypoint_loss)  # keypoint_objs_weight shape: (val_objs, 10, 2)
                        valid_keypoint_objs_weight = keypoint_objs_weight[keypoint_loss_mask]  # valid_keypoint_objs_weight shape: (valid_equas,)
                    ...
                    # N x K x 3
                    keypoint_loss = self.keypoint_loss_fnc(preds['keypoints'], pred_targets['keypoints']).sum(2) * \
                ...
                if self.compute_keypoint_depth_loss:
                    ...
                    valid_keypoint_depth_loss = self.loss_weights['keypoint_depth_loss'] * self.reg_loss_fnc(
                        valid_pred_keypoints_depth, target_keypoints_depth[keypoints_depth_mask])

                    if (~keypoints_depth_mask.sum()) > 0:
                        invalid_keypoint_depth_loss = self.loss_weights['keypoint_depth_loss'] * self.reg_loss_fnc(
                            invalid_pred_keypoints_depth, target_keypoints_depth[~keypoints_depth_mask])
                    else:
                        invalid_keypoint_depth_loss = ms.Tensor(0, ms.float32)
                    ...
                    if self.corner_with_uncertainty:
                        ...
                        if ~keypoints_depth_mask.sum() > 0:
                            ...
                            invalid_uncertainty = ms.Tensor(0, ms.float32)
                    ...
                    # average
                    valid_keypoint_depth_loss = valid_keypoint_depth_loss.sum() / ops.clamp(keypoints_depth_mask.sum(),1)
                    invalid_keypoint_depth_loss = invalid_keypoint_depth_loss.sum() / ops.clamp((~keypoints_depth_mask).sum(), 1)  # all ~.problem
                ...
                if self.corner_with_uncertainty:
                    if self.pred_direct_depth and self.depth_with_uncertainty:
                        ...
                        combined_uncertainty = self.concat((ops.expand_dims(preds['depth_uncertainty'], 1), preds['corner_offset_uncertainty'])).exp()
            ...
            if self.compute_combined_depth_loss:  # The loss for final estimated depth.
                ...
                combined_depth_loss = self.loss_weight_ramper(iteration) * self.loss_weights['combined_depth_loss'] * combined_depth_loss.mean()
            ...
            if self.compute_GRM_loss:
                ...
                invalid_GRM_loss = self.reg_loss_fnc(ops.reduce_sum((invalid_GRM_A * invalid_target_location), 1),
                                                     invalid_GRM_B.squeeze(
                                                         1))  # invalid_GRM_loss shape: (invalid_equas, 1)
            ...
            if self.compute_SoftGRM_loss:
                ...
                valid_SoftGRM_loss = self.reg_loss_fnc(valid_separate_depths,
                                                       valid_target_depth) / valid_uncern + self.log(valid_uncern)
                if self.cfg.SOLVER.DYNAMIC_WEIGHT:
                ...
                valid_SoftGRM_loss = (valid_SoftGRM_loss * valid_SoftGRM_weight).sum()/ops.clamp(GRM_valid_items.sum(), 1)
                if (~GRM_valid_items).sum() > 0:
                    invalid_SoftGRM_loss = self.div(self.reg_loss_fnc(ops.Tensor(invalid_separate_depths), invalid_target_depth), invalid_uncern)
                    invalid_SoftGRM_loss = self.div((invalid_SoftGRM_loss * invalid_SoftGRM_weight).sum(), ops.clamp((~GRM_valid_items).sum(),1))  # Avoid the occasion that no invalid equations and the returned value is NaN.
                else:
                    invalid_SoftGRM_loss = ms.Tensor(0,ms.float32)  # problem,all ~. Avoid the occasion that no invalid equations and the returned value is NaN.
...
class Anno_Encoder(nn.Cell):
    ...
    def decode_from_GRM(self, pred_rotys, pred_dimensions, pred_keypoint_offset, pred_direct_depths, targets_dict,
        ...
        kp_group = ops.cat([(ops.Reshape(n_pred_keypoints, (total_num_objs, 20)), n_centers_3D,
                             ops.Zeros((total_num_objs, 2), ms.float32))],
                           axis=1)  # kp_group shape: (total_num_objs, 24)
