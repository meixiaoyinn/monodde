        ...
        for idx, regress_head_key in enumerate(self.regression_head_cfg):
            if self.use_inplace_abn:
                feat_layer = nn.SequentialCell([nn.Conv2d(in_channels, self.head_conv, kernel_size=3, pad_mode='pad',padding=1),
                                               nn.BatchNorm2d(num_features=self.head_conv),   #need to use InPlaceABN
                                               nn.LeakyReLU()])
            else:
                feat_layer = nn.SequentialCell([nn.Conv2d(in_channels, self.head_conv, kernel_size=3, pad_mode='pad',padding=1),
            ...
            for key_index, key in enumerate(regress_head_key):
                ...
                if key.find('uncertainty') >= 0 and cfg.MODEL.HEAD.UNCERTAINTY_INIT:
                    if isinstance(output_head, nn.SequentialCell):
                        output_head[-1].weight.set_data(initializer(ms.common.initializer.XavierNormal(gain=0.01),output_head[-1].weight.shape, output_head[-1].weight.dtype))
                    else:
                        output_head.weight.set_data(ms.common.initializer.initializer(ms.common.initializer.XavierNormal(gain=0.01),output_head.weight.shape, output_head.weight.dtype))
            ...
            for j, reg_output_head in enumerate(self.reg_heads[i]):
                ...
                if self.enable_edge_fusion and i == self.offset_index[0] and j == self.offset_index[1]:
                    ...
                    for k in range(b):
                        ...
                        output_reg[k, :,edge_indice_k[:, 1],edge_indice_k[:, 0]] += ops.permute(edge_offset_output[k, :,:edge_lens[k]],(1,0))
...
def _fill_fc_weights(layers):
    for name, param in layers.parameters_and_names():
        if isinstance(param, nn.Conv2d):
            if 'bias' in name:
                param.weight.set_data(initializer('zeros',param.shape,param.dtype))