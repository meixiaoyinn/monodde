...
class BasicBlock(nn.Cell):
    def __init__(self, inplanes, planes, stride=1, dilation=1, norm_type = 'BIN'):
        ...
        self.bn1=get_norm(planes,norm_type=norm_type,momentum=BN_MOMENTUM)
        self.relu=nn.ReLU()
        self.conv2=nn.Conv2d(planes,planes, kernel_size=3,
                               stride=1, pad_mode='pad',padding=dilation,
                               has_bias=False, dilation=dilation)
        self.bn2=get_norm(planes,norm_type=norm_type,momentum=BN_MOMENTUM)
...
class batch_instance_norm(nn.Cell):
    def __init__(self, planes, momentum = 0.9):
        ...
        self.BN2 = nn.BatchNorm2d(BATCH_SIZE*planes, momentum=momentum)
...
class Con_norm_act(nn.Cell):
    def __init__(self,in_channel,out_channel,kernel_size,stride=1,pad_mode='pad',padding=0,has_bias=False,dilation=1,momentum=0.9,norm_type='BN'):
...
class Root(nn.Cell):
    def __init__(self, in_channels, out_channels, kernel_size, residual):
        ...
        self.bn=nn.BatchNorm2d(out_channels,BN_MOMENTUM)
        self.relu = nn.ReLU()
...
class DeformConv2d(nn.Cell):
    ...
    def __init__(self, inc, outc, kernel_size=3, stride=1, pad_mode='pad', padding=0, has_bias=False, modulation=True):
        ...
        self.conv.weight.set_data(initializer(XavierUniform(0.02), self.conv.weight.shape))
        ...
        self.p_conv.weight.set_data(initializer(XavierUniform(0.02), self.p_conv.weight.shape))
        if modulation:
