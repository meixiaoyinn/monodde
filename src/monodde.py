...
class MonoddeWithLossCell(nn.Cell):
    ...
    def prepare_predictions(self, targets_variables, predictions):
        ...
        target_regression_2D = self.concat((valid_targets_bbox_points - target_bboxes_2D[:, :2], target_bboxes_2D[:, 2:] - valid_targets_bbox_points))  # offset to 2D bbox boundaries.
        ...
        if self.corner_loss_depth == 'GRM':
            ...
            GRM_valid_items = self.concat((keypoints_visible,self.ones((keypoints_visible.shape[0], 3), ms.bool_)))  # GRM_valid_items shape: (valid_objs, 25)
        ...
        if self.dim_uncern:
            ...
            preds['dim_uncern'] = ops.exp(ops.clamp(dim_uncern, self.uncertainty_range[0],self.uncertainty_range[1]))
        ...
        if self.combined_depth_uncern:
            ...
            preds['combined_depth_uncern'] = ops.clamp(combined_depth_uncern, self.uncertainty_range[0],self.uncertainty_range[1]).exp()
        ...
        if self.corner_loss_uncern:
            ...
            preds['corner_loss_uncern'] = ops.clamp(corner_loss_uncern, self.uncertainty_range[0],self.uncertainty_range[1]).exp()

        # predict the uncertainties of the solved depths from groups of keypoints
        if self.corner_with_uncertainty:
            preds['corner_offset_uncertainty'] = pred_regression_pois_3D[:, self.key2channel('corner_uncertainty')]  # preds['corner_offset_uncertainty'] shape: (val_objs, 3)

            if self.uncertainty_range is not None:
                preds['corner_offset_uncertainty'] = ops.clamp(preds['corner_offset_uncertainty'],self.uncertainty_range[0],self.uncertainty_range[1])

        if self.corner_loss_depth == 'GRM':
            GRM_uncern = pred_regression_pois_3D[:, self.key2channel('GRM_uncern')]  # GRM_uncern shape: (num_objs, 25)
            GRM_uncern = ops.exp(ops.clamp(GRM_uncern, self.uncertainty_range[0], self.uncertainty_range[1]))
            # Decode rot_y
            ...
            pred_combined_uncertainty = ops.cat(ops.exp((ops.expand_dims(preds['depth_uncertainty'],-1), preds['corner_offset_uncertainty'])),axis=1)  # pred_combined_uncertainty shape: (val_objs, 4)
            pred_combined_depths = ops.cat((ops.expand_dims(pred_direct_depths_3D,-1), preds['keypoints_depths']),axis=1)  # pred_combined_depths shape: (val_objs, 4)

            if self.corner_loss_depth == 'soft_combine':  # Weighted sum.
                pred_uncertainty_weights = 1 / pred_combined_uncertainty
                pred_uncertainty_weights = pred_uncertainty_weights / ops.sum(pred_uncertainty_weights,keepdim=True,dim=1)
                pred_corner_depth_3D = ops.sum(pred_combined_depths * pred_uncertainty_weights, 1)
                ...
                pred_corner_depth_3D = pred_combined_depths[
                    ops.arange(pred_combined_depths.shape[0]), pred_combined_uncertainty.argmin(axis=1)]
        ...
        pred_rotys_3D, _ = self.anno_encoder.decode_axes_orientation(pred_orientation_3D, pred_locations_3D)  # pred_rotys_3D shape: (val_objs,)
        # encode corners
        pred_corners_3D = self.anno_encoder.encode_box3d(pred_rotys_3D, pred_dimensions_3D, pred_locations_3D)  # pred_corners_3D shape: (val_objs, 8, 3)
