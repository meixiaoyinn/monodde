...
class EvalWrapper:
    ...
    def prepare_targets(self, data):
        ...
        return_dict = dict(cls_ids=cls_ids, size=size, target_centers=target_centers, bboxes=bboxes,
                           keypoints=keypoints,
                           ...
                           img_ids=ops.Tensor(data['original_idx'], ms.int32)
                           )
    ...
    def inference_once(self, data, iteration):
        ...
        pred_regression_pois = select_point_of_interest(batch, indexs, pred_regression).view(-1,
                                                                                             pred_regression.shape[1])
        ...
        if self.output_depth == 'GRM':
            ...
            GRM_rotys, GRM_alphas = self.anno_encoder.decode_axes_orientation(pred_orientation,
            ...
            if 'GRM_uncern' in self.key2channel.keys:
                ...
                uncern_Mono_Direct = pred_regression_pois[:, self.key2channel(
                    'Mono_Direct_uncern')]  # uncern_Mono_Direct shape: (valid_objs, 1)
                uncern_Mono_Keypoint = pred_regression_pois[:, self.key2channel(
                    'Mono_Keypoint_uncern')]  # uncern_Mono_Keypoint shape: (valid_objs, 3)
                GRM_uncern = ops.cat((ops.expand_dims(uncern_GRM1, 2), ops.expand_dims(uncern_GRM2, 2)), axis=2).view(
                    -1, 16)
                GRM_uncern = ops.cat((GRM_uncern, uncern_Mono_Direct, uncern_Mono_Keypoint),
                                     axis=1)  # GRM_uncern shape: (valid_objs, 20)
            GRM_uncern = ops.clamp(GRM_uncern, min=self.uncertainty_range[0], max=self.uncertainty_range[1]).exp()
            ...
            pred_combined_depths = ops.cat((ops.expand_dims(pred_direct_depths, 1), pred_keypoints_depths),
                                           axis=1)  # pred_combined_depths shape: (valid_objs, 4)
            ...
            if self.cfg.TEST.UNCERTAINTY_3D == "GRM_uncern":
                ...
                combined_depth_uncern = ops.clamp(combined_depth_uncern, min=self.uncertainty_range[0],
                                                  max=self.uncertainty_range[1]).exp()
                ...
                corner_loss_uncern = ops.clamp(corner_loss_uncern, min=self.uncertainty_range[0],
                                               max=self.uncertainty_range[1]).exp()
                ...
                estimated_depth_error = ops.cat(
                    (GRM_error.unsqueeze(1), ops.expand_dims(combined_depth_error[:GRM_error.shape[0]], 1),
                     ops.expand_dims(corner_loss_error[:GRM_error.shape[0]], 1)), axis=1)
        ...
        # hard ensemble, soft ensemble and simple average
            ...
            if self.output_depth == 'hard':
                pred_depths = pred_combined_depths[
                    ops.arange(pred_combined_depths.shape[0]), depth_weights.argmax(axis=1)]
    ...
    def evaluate_3D_depths(self, targets, pred_regression):
        ...
        pred_keypoint_offset = pred_regression_pois[:,
                               self.key2channel('corner_offset')]  # pred_keypoint_offset: (num_objs, 20)

        # Orientatiion predictions
        pred_orientation = ops.cat((pred_regression_pois[:, self.key2channel('ori_cls')],
                                    pred_regression_pois[:, self.key2channel('ori_offset')]),
                                   axis=1)  # pred_orientation shape: (num_objs, 16)
        ...
        target_clses = targets['cls_ids'].view(-1)[reg_mask]
        pred_dimensions_offsets = pred_regression_pois[:, self.key2channel('3d_dim')]
        pred_dimensions = self.anno_encoder.decode_dimension(target_clses,
                                                             pred_dimensions_offsets, )  # pred_dimensions shape: (num_objs, 3)
        # direct
        pred_direct_depths = self.anno_encoder.decode_depth(
            pred_depths_offset.squeeze(-1))  # pred_direct_depths shape: (num_objs,)
        # three depths from keypoints
        pred_keypoints_depths = self.anno_encoder.decode_depth_from_keypoints_batch(
            pred_keypoint_offset.view(-1, 10, 2), pred_dimensions,
            targets['calib'])  # pred_keypoints_depths shape: (num_objs, 3)
        # The depth solved by original MonoFlex.
        pred_combined_depths = ops.cat((pred_direct_depths.unsqueeze(1), pred_keypoints_depths),
                                       axis=1)  # pred_combined_depths shape: (num_objs, 4)
        ...
        if MonoFlex_depth_flag:
            ...
            pred_uncertainty_min_depth = pred_combined_depths[
                ops.arange(pred_combined_depths.shape[0]), pred_combined_uncertainty.argmin(
                    axis=1)]  # Select the depth with the smallest uncertainty.
            ...
            pred_uncertainty_softmax_depth = ops.sum(pred_combined_depths * pred_uncertainty_weights,
                                                     axis=1)  # Depth produced by soft weighting.
    ...
    def survey_depth_statistics(self, targets, pred_regression, image_ids):
        ...
        if self.depth_with_uncertainty:
            pred_direct_uncertainty = pred_regression_pois[:, self.key2channel(
                'depth_uncertainty')].exp()  # pred_direct_uncertainty shape: (num_objs, 1)
        else:
            pred_direct_uncertainty = None
        if self.keypoint_depth_with_uncertainty:
            pred_keypoint_uncertainty = pred_regression_pois[:, self.key2channel(
                'corner_uncertainty')].exp()  # pred_keypoint_uncertainty shape: (num_objs, 3)
        else:
        ...
        dimension_residual = (pred_dimensions - targets['dimensions'][0, reg_mask, :]) / targets['dimensions'][0,
                                                                                         reg_mask,
                                                                                         :]  # dimension_residual shape: (num_objs, 3). (l, h, w)
        target_keypoint_offset = targets['keypoints'][0, :, :, 0:2]
        target_keypoint_offset = target_keypoint_offset[reg_mask].view(-1,
                                                                       20)  # Offset from target centers to keypoints.
        target_vertex_offset = target_keypoint_offset[:, 0:16]

        # direct
        pred_direct_depths = ops.expand_dims(self.anno_encoder.decode_depth(pred_depths_offset.squeeze(-1)),
                                             1)  # pred_direct_depths shape: (num_objs, 1)
        # three depths from keypoints
        pred_keypoints_depths = self.anno_encoder.decode_depth_from_keypoints_batch(
            pred_keypoint_offset.view(-1, 10, 2), pred_dimensions,
            targets['calib'])  # pred_keypoints_depths shape: (num_objs, 3)
        ...
        pred_combined_depths = ops.cat((pred_direct_depths, pred_keypoints_depths),
                                       axis=1)  # pred_combined_depths shape: (valid_objs, 4)
        info_dict = {'target_centers': target_points, 'offset_3D': pred_offset_3D, 'pad_size': targets['pad_size'],
                     'calib': targets['calib'], 'batch_idxs': None}
        GRM_rotys, GRM_alphas = self.anno_encoder.decode_axes_orientation(pred_orientation,
        ...
        if 'GRM_uncern' in self.key2channel.keys:
            ...
            uncern_Mono_Direct = pred_regression_pois[:,
                                 self.key2channel('Mono_Direct_uncern')]  # uncern_Mono_Direct shape: (valid_objs, 1)
            uncern_Mono_Keypoint = pred_regression_pois[:, self.key2channel(
                'Mono_Keypoint_uncern')]  # uncern_Mono_Keypoint shape: (valid_objs, 3)
            GRM_uncern = ops.cat((uncern_GRM1.unsqueeze(2), uncern_GRM2.unsqueeze(2)), axis=2).view(-1, 16)
            GRM_uncern = ops.cat((GRM_uncern, uncern_Mono_Direct, uncern_Mono_Keypoint),
